...
export async function GET(
  ...
  try {
    ...
    // Récupérer l'historique des statuts depuis les notifications
    ...
    // Construire l'historique des statuts
    ...
    // Traiter les notifications pour construire l'historique
    if (notifications) {
      notifications.forEach((notification) => {
        // Si c'est un changement de statut
        if (notification.type === 'status_change') {
          statusHistory.push({
            status: notification.data?.new_status || order.status,
        ...
        messages.push({
          ...
          type: notification.type === 'error' ? 'error' :
                ...
                notification.type === 'status_change' ? 'success' : 'info',
          from: notification.data?.from || 'system'
        });
      });
    }

    // S'assurer que le statut actuel est dans l'historique
    ...
    if (order.delivery_address) {
      try {
        ...
        console.error('Erreur lors du parsing de l\'adresse:', e);
      }
    }
...
// Endpoint pour mettre à jour le statut (pour les admins)
export async function PUT(
  ...
  try {
    ...
    const { status, message, estimatedDelivery } = body;
    ...
    // Créer une notification pour le changement
    if (status || message) {
      const { error: notificationError } = await supabase
        .from('order_notifications')
        .insert({
          order_id: orderId,
          type: status ? 'status_change' : 'info',
          message: message || getStatusMessage(status),
          data: {
            old_status: order.status,
            new_status: status,
            from: isAdmin ? 'admin' : 'system'
          }
        });
