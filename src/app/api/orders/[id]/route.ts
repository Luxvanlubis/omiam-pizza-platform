...
export async function GET(
  ...
  try {
    ...
    const { data: order, error: orderError } = await supabaseIntegrationService.getSupabaseClient()
      ...
      .limit(10, { referencedTable: 'notifications' })
      .single();
    ...
    console.error('Erreur lors de la récupération de la commande:', error instanceof Error ? error.message : String(error));
    return NextResponse.json({
...
export async function PUT(
  ...
  try {
    ...
    const { data: existingOrder, error: orderError } = await supabaseIntegrationService.getSupabaseClient()
      .from('orders')
      .select(`
        *,
        profiles (
    ...
    if (isOwner && !isAdmin) {
      ...
      if (unauthorizedFields.length > 0) {
        return NextResponse.json(
          { error: `Champs non autorisés: ${unauthorizedFields.join(', ')}` },
          { status: 403 }
        );
      }
    }
    ...
    const { data: updatedOrder, error: updateError } = await supabaseIntegrationService.getSupabaseClient()
      .from('orders')
      .update(updateData)
      .eq('id', orderId)
      .select(`
    ...
    if (body.status && body.status !== existingOrder.status && (body.notifyCustomer !== false)) {
      try {
        await OrderNotificationMiddleware.handleOrderStatusChange({
          orderId,
          ...
          metadata: {
            ...
            updatedByRole: session.user.user_metadata?.role,
            Notes: body.Notes
          }
        });
        // Notification en temps réel gérée par OrderNotificationMiddleware
    ...
    const orderWithParsedData = {
      ...updatedOrder,
      delivery_address: updatedOrder.delivery_address ? JSON.parse(updatedOrder.delivery_address) : null,
      items: updatedOrder.order_items?.map(item => ({
        ...item,
        product: item.products,
        customizations: item.customizations ? JSON.parse(item.customizations) : {}
      })) || [],
      user: updatedOrder.profiles
    };

    return NextResponse.json(orderWithParsedData);
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    const { error: deleteError } = await supabaseIntegrationService.getSupabaseClient()
      .from('orders')
      .delete()
      .eq('id', orderId);
